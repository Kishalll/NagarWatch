...
const Dashboard = () => {
    ...
    const handleAdminAddHouse = async (e) => {
        ...
        await addDoc(collection(db, 'houses'), { ...data, ...newHousePos, userId: 'admin_created', createdAt: new Date().toISOString() });
            ...
            <main className="flex-1 relative overflow-hidden bg-slate-100">
                ...
                {activeTab === 'map' && (
                    <div className="h-full w-full relative z-0">
                        ...
                        {isAddingHouseMode && (
                            <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-2 rounded-full shadow-lg z-[1000] font-bold text-sm animate-pulse">
                                Tap map to place new house
                            </div>
                        )}
                        ...
                        {isAddingHouseMode && newHousePos && (
                            <div className="absolute bottom-0 left-0 right-0 bg-white p-6 rounded-t-3xl shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-[1000] animate-in slide-in-from-bottom duration-300">
                                <form onSubmit={handleAdminAddHouse} className="space-y-3">
                                    <div className="flex justify-between items-center mb-2"><h3 className="font-bold text-slate-800">New House Details</h3></div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <input name="number" required placeholder="House No" className="border p-3 rounded-xl w-full bg-slate-50" />
                                        <input name="ownerName" required placeholder="Owner Name" className="border p-3 rounded-xl w-full bg-slate-50" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <input name="occupants" type="number" placeholder="Occupants" className="border p-3 rounded-xl w-full bg-slate-50" />
                                        <input name="contact" placeholder="Mobile" className="border p-3 rounded-xl w-full bg-slate-50" />
                                    </div>
                                    <select name="status" className="w-full border p-3 rounded-xl bg-slate-50">
                                        <option value="occupied">Occupied</option>
                                        <option value="vacant_rent">For Rent</option>
                                        <option value="vacant_sale">For Sale</option>
                                        <option value="away">Away</option>
                                    </select>
                                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl">Add House to Map</button>
                                </form>
                            </div>
                        )}
                    </div>
